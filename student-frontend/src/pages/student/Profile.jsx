...
import { Camera, Save, Lock, User, Mail, BookOpen, Award, TrendingUp, Calendar, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-[var(--muted)] mt-1">Manage your account details and security settings</p>
            </div>
            ...
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                        ...
                        <span className="inline-block mt-2 px-3 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/20 text-indigo-400 capitalize">
                            {user?.role}
                        </span>
                        {user?.bio && (
                ...
                {stats && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-5 border-t border-[var(--border)]">
                        <StatPill icon={BookOpen} label="Enrolled" value={stats.enrolled_courses} color="blue" />
                        <StatPill icon={CheckCircle} label="Completed" value={stats.completed_courses} color="green" />
                        <StatPill icon={TrendingUp} label="Avg Quiz" value={`${Math.round(stats.avg_quiz_score)}%`} color="purple" />
                        <StatPill icon={Award} label="Badges" value={stats.badges_earned} color="yellow" />
                    </div>
                )}
            </div>
                ...
                <form onSubmit={handleProfileSave} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-2.5 rounded-xl bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text)] text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                    ...
                    <div>
                        <label className="block text-sm text-[var(--muted)] mb-1">Bio <span className="text-xs">(optional)</span></label>
                        <textarea
                            value={form.bio}
                            onChange={e => setForm(f => ({ ...f, bio: e.target.value }))}
                            rows={3}
                            maxLength={300}
                            className="w-full px-4 py-2.5 rounded-xl bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text)] text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition resize-none"
                        ...
                        <div key={key}>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 pr-12 rounded-xl bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text)] text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                                    placeholder="••••••••"
