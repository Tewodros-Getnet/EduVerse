...
export default function Assignments() {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'submitted': return 'bg-blue-500/20  text-blue-300  border border-[var(--accent-tertiary)]/30';
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            {(assignments || []).length === 0 ? (
                <div className="bg-[var(--surface)] border border-purple-900/30 rounded-2xl p-8 text-center text-[var(--muted)]">
                    ...
                    <h3 className="text-lg font-medium text-[var(--text)] mb-2">No assignments yet</h3>
                            ...
                            <div key={assignment.id} className="bg-[var(--surface)] border border-purple-900/30 rounded-2xl p-5">
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-[var(--muted)] mb-3">{assignment.description}</p>

                                        {/* Instructions — shown prominently so student reads before submitting */}
                                        {assignment.instructions && (
                                            <div className="mb-3 bg-[var(--surface-2)] border border-purple-900/20 rounded-xl p-3">
                                                <p className="text-xs text-[var(--accent-primary)]/80 font-semibold mb-1.5">📋 Assignment Instructions</p>
                                                <p className="text-sm text-[var(--muted)] whitespace-pre-wrap leading-relaxed line-clamp-4">
                                                    {assignment.instructions}
                                                </p>
                                            </div>
                                        )}
                                        ...
                                        {assignment.attachment_url && (
                                            <a href={assignment.attachment_url} target="_blank" rel="noopener noreferrer"
                                                className="inline-flex items-center gap-2 mb-3 px-3 py-2 bg-blue-500/10 border border-[var(--accent-tertiary)]/30 rounded-xl text-blue-300 text-xs font-medium hover:bg-blue-500/20 transition">
                                    ...
                                    <div className="bg-[var(--surface-2)] rounded-xl p-4">
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[var(--muted)] font-medium mb-2 text-sm">Your Submission</p>
                                                {submission.content && (
                                                    <p className="text-[var(--muted)] text-sm line-clamp-3">{submission.content}</p>
                                                )}
                                                {submission.file_url && (
                                                ...
                                                {!submission.content && !submission.file_url && (
                                                    <p className="text-[var(--muted)] text-sm italic">No content</p>
                                                )}
                                            </div>
                                            <button
                                                onClick={() => viewSubmissionDetails(assignment, submission)}
                                                className="ml-3 flex-shrink-0 px-3 py-1 bg-[var(--accent-primary)]/30 border border-[var(--accent-primary)]/30 rounded-lg text-xs text-[var(--accent-primary)]/80 hover:bg-[var(--accent-primary)]/40 transition"
                                            >
                                                View Details
                                            </button>
                                        </div>
                                        ...
                                        {submission.feedback && (
                                            <div className="mt-3 pt-3 border-t border-purple-900/30">
                                                <p className="text-[var(--accent-primary)]/80 text-xs font-medium mb-1">Instructor Feedback:</p>
                                                <p className="text-[var(--muted)] text-sm">{submission.feedback}</p>
                                            </div>
                                        )}

                                        {submission.score !== null && (
                                            <div className="mt-3 pt-3 border-t border-purple-900/30 flex items-center justify-between">
                                                <span className="text-[var(--muted)] text-sm">Grade:</span>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-[var(--surface)] border border-purple-900/30 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between p-5 border-b border-purple-900/30">
                            <div>
                                <h2 className="font-semibold text-[var(--text)]">{selectedAssignment.title}</h2>
                        ...
                        <div className="p-5 space-y-5">
                            ...
                            {selectedAssignment.instructions ? (
                                <div className="bg-[var(--surface-2)] border border-purple-900/20 rounded-xl p-4">
                                    <p className="text-xs text-[var(--accent-primary)]/80 font-semibold mb-2">📋 Assignment Instructions</p>
                                ...
                                <div className="bg-[var(--surface-2)] rounded-xl p-4">
                                    <p className="text-xs text-[var(--accent-primary)]/80 font-medium mb-1">Assignment Description</p>
                                    <p className="text-sm text-[var(--muted)]">{selectedAssignment.description}</p>
                                </div>
                            ) : null}
                            ...
                            {selectedAssignment.attachment_url && (
                                <a href={selectedAssignment.attachment_url} target="_blank" rel="noopener noreferrer"
                                    className="flex items-center gap-3 px-4 py-3 bg-blue-500/10 border border-[var(--accent-tertiary)]/30 rounded-xl text-blue-300 hover:bg-blue-500/20 transition">
                                    <span className="text-xl">📎</span>
                                    <div>
                                        <p className="text-sm font-medium">{selectedAssignment.attachment_name || 'Assignment Brief'}</p>
                                        <p className="text-xs text-[var(--muted)]">Click to open / download</p>
                                    </div>
                            ...
                            {/* Text answer */}
                            <div>
                                <label className="block text-sm text-[var(--muted)] font-medium mb-2">
                                    Your Answer <span className="text-[var(--muted)] font-normal">(optional if you attach a file)</span>
                                </label>
                                <textarea
                                    ...
                                    rows={6}
                                    className="w-full bg-[var(--surface-2)] border border-[var(--border)]/40 rounded-xl px-4 py-3 text-[var(--text)] placeholder-[var(--muted)] focus:outline-none focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-purple-500/20 text-sm resize-none transition"
                                />
                            </div>
                            ...
                            <div>
                                <label className="block text-sm text-[var(--muted)] font-medium mb-2">
                                    Attach a File <span className="text-[var(--muted)] font-normal">(PDF, Word, image — max 20 MB)</span>
                                </label>
                                    ...
                                    <div className="flex items-center justify-between bg-[var(--surface-2)] border border-green-500/30 rounded-xl px-4 py-3">
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <p className="text-sm text-[var(--text)] truncate">{uploadedFile.name}</p>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-[var(--surface)] border border-purple-900/30 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-5 space-y-4">
                            <div>
                                <h3 className="text-[var(--text)] font-medium mb-2">{viewingSubmission.assignment.title}</h3>
                            ...
                            {viewingSubmission.submission.content && (
                                <div className="bg-[var(--surface-2)] rounded-xl p-4">
                                    <p className="text-xs text-[var(--accent-primary)]/80 font-medium mb-2">Your Answer</p>
                            ...
                            {viewingSubmission.submission.file_url && (
                                <div className="bg-[var(--surface-2)] rounded-xl p-4 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <span className="text-xl">📎</span>
                                        <p className="text-sm text-[var(--text)]">Attached file</p>
                                    </div>
                            ...
                            {viewingSubmission.submission.feedback && (
                                <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4">
                                    ...
                                    <p className="text-[var(--muted)] text-sm">{viewingSubmission.submission.feedback}</p>
                                </div>
                            )}
                        </div>
                    </div>
