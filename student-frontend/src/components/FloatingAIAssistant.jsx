        ...
        <>
            ...
            <button
                ...
                className={`fixed bottom-20 right-4 md:bottom-6 md:right-6 w-14 h-14 rounded-full shadow-lg transition-all duration-300 z-40 flex items-center justify-center hover:scale-110 ${isOpen
                        ? 'bg-gradient-to-br from-indigo-600 to-[var(--accent-primary)]/80'
                        : 'bg-gradient-to-br from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600'
                    }`}
                title="AI Tutor Assistant"
            >
                <Brain className="w-6 h-6 text-[var(--text)]" />
            </button>
                ...
                <div className="fixed bottom-32 right-4 md:bottom-24 md:right-6 w-[calc(100vw-2rem)] max-w-sm sm:max-w-md h-[min(600px,70vh)] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col z-50 border border-gray-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-indigo-600 to-[var(--accent-primary)]/80 text-[var(--text)] px-6 py-4 flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="text-[var(--text)] hover:text-indigo-100 transition"
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-slate-800">
                            ...
                            >
                                <div
                                    className={`max-w-xs px-4 py-3 rounded-xl ${msg.role === 'user'
                                            ? 'bg-indigo-600 text-[var(--text)] rounded-br-none'
                                            : 'bg-white dark:bg-slate-700 text-[var(--text)] dark:text-[var(--text)] rounded-bl-none shadow-sm'
                                        }`}
                                >
                        ...
                        {loading && (
                            <div className="flex justify-start">
                                <div className="bg-white dark:bg-slate-700 text-[var(--text)] dark:text-[var(--text)] px-4 py-3 rounded-xl rounded-bl-none">
                    ...
                    {messages.length <= 2 && !loading && (
                        <div className="border-t border-gray-200 dark:border-slate-700 p-3 bg-gray-50 dark:bg-slate-800">
                            <p className="text-xs text-gray-600 dark:text-[var(--muted)] mb-2">Quick actions:</p>
                        <div className="grid grid-cols-2 gap-2">
                                ...
                                <button
                                    ...
                                    className="text-xs px-2 py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-[var(--accent-primary)]/80 rounded-lg hover:bg-purple-200 dark:hover:bg-purple-900/50 transition flex items-center justify-center gap-1"
                                >
                    ...
                    <form onSubmit={handleSendMessage} className="border-t border-gray-200 dark:border-slate-700 p-4 bg-white dark:bg-slate-900">
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 px-3 py-2 rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-[var(--text)] dark:text-[var(--text)] placeholder-gray-400 dark:placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                                disabled={loading}
                            />
                            <button
                                type="submit"
                                disabled={loading || !input.trim()}
                                className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-[var(--accent-primary)]/80 text-[var(--text)] rounded-xl font-medium hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                            >
