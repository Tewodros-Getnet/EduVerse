...
import { Bell, BellRing, Check, Settings, X, Users, DollarSign } from 'lucide-react';
import api from '../api/axios';
import toast from 'react-hot-toast';

export default function InstructorNotificationButton() {
    ...
    const fetchNotifications = async () => {
        try {
            const res = await api.get('/notifications/instructor');
    ...
    const markAllAsRead = async () => {
        try {
            await api.post('/notifications/instructor/read-all');
    ...
    const filteredNotifications = notifications.filter(notification => {
        ...
        if (activeTab === 'enrollments') return notification.type === 'enrollment';
        if (activeTab === 'assignments') return notification.type === 'assignment';
        if (activeTab === 'revenue') return notification.type === 'revenue';
        return true;
    });
                ...
                <div className="absolute right-0 mt-2 w-96 bg-[#1a1a35] border border-purple-900/40 rounded-2xl shadow-2xl z-50 overflow-hidden">
                    ...
                    <div className="p-3 border-t border-purple-900/40">
                        <button
                            onClick={() => {
                                setIsOpen(false);
                                // Navigate to full notifications page or generate test notifications
                                api.post('/notifications/instructor/generate').then(() => {
                                    fetchNotifications();
